...
label=labelNormalizeAndNoise(label,0.8,0.26,0.1)#=========================================
data =np.concatenate((inputDatas, label),axis=1)
print(data)
train,test=separateDataAsTrainAndTest(data,0.7)
print(train.shape)
print(test.shape)
saveToCsv(data,'../datas/csvZmd_All.csv')
saveToCsv(train,'../datas/TrainData/csvZmd_Train.csv')
saveToCsv(test,'../datas/TestData/csvZmd_Test.csv')
